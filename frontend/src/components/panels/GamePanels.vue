<script setup>
    ...
    // Plugin-owned self attributes are open-ended.
    const attributes = computed(() => me?.value?.attributes ?? []);
        ...
        <div id="right-dashboard" class="text-sm">
            <div class="fixed z-10 w-80 right-2 top-4 max-h-full">
                <div class="flex flex-col h-full space-y-2">
                    ...
                    <!-- Plugin-owned attributes (hidden when empty) -->
                    <div class="z-10 bg-neutral rounded-lg px-3 py-1.5 text-xs w-80 opacity-80 hover:opacity-100"
                         v-if="attributes.length">
                        <div class="font-medium mb-1">Agent state</div>
                        <div v-for="attribute in attributes" :key="attribute.kind" class="flex items-center gap-2 py-0.5">
                            <span class="min-w-0 flex-1 truncate" :title="attribute.kind">{{ attribute.kind }}</span>
